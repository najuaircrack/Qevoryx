...
void StatusPanel::render(const TuiState& state,
    ...
    const int column_width = std::max(rect().width / 3, 24);

    wattron(window, theme.success);
    mvwaddstr(window, first_row, 3, "? Configuration loaded");
    wattroff(window, theme.success);

    wattron(window, theme.success);
    mvwaddstr(window, first_row, 3 + column_width, "? Interface detected");
    wattroff(window, theme.success);

    wattron(window, snapshot.running ? theme.success : theme.accent);
    mvwaddstr(window, first_row, 3 + column_width * 2, snapshot.running ? "? Running" : "? Ready");
